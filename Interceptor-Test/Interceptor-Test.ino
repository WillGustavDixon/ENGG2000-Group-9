...
const float SPR = 700; // States per revolution
const float CPD = SPR/360;  // Counts per degree (should be 1.9444)
const int SPEED = 128;
...
void rotateToPos(float targ) {
  long targPulses = targ*CPD;
  long error = targPulses - totalMotorPulses;
  long curPos = totalMotorPulses % 700;
  bool anti;
  int timer = millis();
  targ < 0 ? anti = true : anti = false;  // if targ is negative, we move anticlockwise
  startMotor(anti);
  while (curPos != targPulses && (millis() - timer < 1000)) {
    curPos = totalMotorPulses % 700;
...
void loop() {
  if (millis() - lastCheck >= 100) {
    ...
    if (foundIR) {  // if an IR signal is detected
      float deg = getMotorDeg();
      Serial.print("Found IR Signal at receiver  " + foundRec + ",  move to  ");
      Serial.print(steps);
      Serial.print("  from  ");
      Serial.println(deg);
      //rotateToPos(steps);
